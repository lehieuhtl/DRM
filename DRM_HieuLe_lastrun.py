...
expName = 'DRM_HieuLe'  # from the Builder filename that created this script
expInfo = {
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/hieule/Desktop/Fall23/AdvTopCog/DRM/DRM_HieuLe_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    text_4 = visual.TextStim(win=win, name='text_4',
        text='Welcome to the DRM Experiment !\n\nPress the spacebar to continue.',
        font='Open Sans',
    ...
    text_2 = visual.TextStim(win=win, name='text_2',
        text='You will now be moving onto the recognition stage',
        font='Open Sans',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='black', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    # --- Initialize components for Routine "blankk" ---
    ...
    WelcomeComponents = [text_4, key_resp_2]
    ...
    list_loop = data.TrialHandler(nReps=1.0, method='random', 
        ...
        trialList=data.importConditions('lists.xlsx'),
    ...
    if thisList_loop != None:
        for paramName in thisList_loop:
            globals()[paramName] = thisList_loop[paramName]
    ...
    for thisList_loop in list_loop:
        ...
        # set up handler to look after randomisation of conditions etc
        encoding_loop = data.TrialHandler(nReps=1.0, method='random', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions(encoding_list),
        ...
        recognition_loop = data.TrialHandler(nReps=1.0, method='random', 
            extraInfo=expInfo, originPath=-1,
            trialList=data.importConditions(recall_list),
            seed=None, name='recognition_loop')
        thisExp.addLoop(recognition_loop)  # add the loop to the experiment
        thisRecognition_loop = recognition_loop.trialList[0]  # so we can initialise stimuli with some values
        # abbreviate parameter names if possible (e.g. rgb = thisRecognition_loop.rgb)
        if thisRecognition_loop != None:
            for paramName in thisRecognition_loop:
                globals()[paramName] = thisRecognition_loop[paramName]
        
        for thisRecognition_loop in recognition_loop:
            currentLoop = recognition_loop
            thisExp.timestampOnFlip(win, 'thisRow.t')
            # pause experiment here if requested
            if thisExp.status == PAUSED:
                pauseExperiment(
            ...
            continueRoutine = True
            # update component parameters for each repeat
            thisExp.addData('recognition.started', globalClock.getTime())
            text_3.setText(word)
            key_resp.keys = []
            key_resp.rt = []
            _key_resp_allKeys = []
            # keep track of which components have finished
            recognitionComponents = [text_3, key_resp]
            ...
            for thisComponent in recognitionComponents:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            thisExp.addData('recognition.stopped', globalClock.getTime())
            # check responses
            ...
            # store data for recognition_loop (TrialHandler)
            recognition_loop.addData('key_resp.keys',key_resp.keys)
            recognition_loop.addData('key_resp.corr', key_resp.corr)
            if key_resp.keys != None:  # we had a response
                recognition_loop.addData('key_resp.rt', key_resp.rt)
                recognition_loop.addData('key_resp.duration', key_resp.duration)
            ...
            if thisSession is not None:
                # if running in a Session with a Liaison client, send data up to now
                thisSession.sendExperimentData()
        # completed 1.0 repeats of 'recognition_loop'
        
        
        # --- Prepare to start Routine "blankk" ---
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('blankk.started', globalClock.getTime())
        # keep track of which components have finished
        blankkComponents = []
        for thisComponent in blankkComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Ending Routine "blankk" ---
        for thisComponent in blankkComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('blankk.stopped', globalClock.getTime())
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
